...
class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    var viewModels: [String: CNotchViewModel] = [:] // UUID -> CNotchViewModel
    var window: NSWindow?
    let vm: CNotchViewModel = .init()
    ...
    private func enableSkyLightOnAllWindows() {
        if Defaults[.showOnAllDisplays] {
            windows.values.forEach { window in
                if let skyWindow = window as? CNotchSkyLightWindow {
        ...
        Task {
            ...
            await MainActor.run {
                if Defaults[.showOnAllDisplays] {
                    ...
                    if let skyWindow = self.window as? CNotchSkyLightWindow {
                        skyWindow.disableSkyLight()
                    }
                }
            }
    ...
    private func createCNotchWindow(for screen: NSScreen, with viewModel: CNotchViewModel) -> NSWindow {
        let size = notchWindowSize(screenUUID: viewModel.screenUUID)
        let rect = NSRect(x: 0, y: 0, width: size.width, height: size.height)
        let styleMask: NSWindow.StyleMask = [.borderless, .nonactivatingPanel, .utilityWindow, .hudWindow]
        
        let window = CNotchSkyLightWindow(contentRect: rect, styleMask: styleMask, backing: .buffered, defer: false)
        let containerView = CNotchFileDropContainerView(frame: rect)
    ...
    private func resizeNotchWindow(for viewModel: CNotchViewModel) {
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        NotificationCenter.default.addObserver(
            ...
            guard let viewModel = notification.object as? CNotchViewModel else { return }
    ...
    func playWelcomeSound() {
        ...
        audioPlayer.play(fileName: "another", fileExtension: "m4a")
    }
    ...
    @objc func adjustWindowPosition(changeAlpha: Bool = false) {
        if Defaults[.showOnAllDisplays] {
            ...
            for screen in NSScreen.screens {
                ...
                if windows[uuid] == nil {
                    let viewModel = CNotchViewModel(screenUUID: uuid)
                    let window = createCNotchWindow(for: screen, with: viewModel)
            ...
            if window == nil {
                window = createCNotchWindow(for: selectedScreen, with: vm)
            }
