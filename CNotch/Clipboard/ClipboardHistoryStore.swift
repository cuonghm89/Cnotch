...
final class ClipboardHistoryStore: ObservableObject {
    ...
    private let storageIdentifier = Bundle.main.bundleIdentifier ?? "CNotch"
    ...
    /// De facto standard (https://nspasteboard.org) that password managers
    /// (1Password, Bitwarden, Keychain's own "copy password", etc.) mark a
    /// pasteboard write with to ask clipboard managers not to record it.
    private static let doNotRecordTypes: Set<NSPasteboard.PasteboardType> = [
    ...
    private func isExcluded(_ bundleID: String?) -> Bool {
        ...
        return Defaults[.clipboardExcludedApps].contains { $0.caseInsensitiveCompare(bundleID) == .orderedSame }
    }

    /// Drops anything already recorded from an app that is now excluded.
    ///
    /// Without this, turning an app off protects only future copies while the
    /// password that prompted it sits in the list -- and adding the default
    /// exclusions in an update would have left every existing one in place.
    private func purgeExcluded() {
        let survivors = entries.filter { !isExcluded($0.sourceAppBundleID) }
        guard survivors.count != entries.count else { return }
        for entry in entries where isExcluded(entry.sourceAppBundleID) {
            removeImageFile(for: entry)
        }
        entries = survivors
        persist()
    ...
    private func captureIfNeeded() {
        ...
        guard presentTypes.isDisjoint(with: Self.doNotRecordTypes) else { return }
        // The frontmost app is a heuristic -- there is no API for who wrote
        // the pasteboard -- but it is the same heuristic used to label the
        // entry, and for a deliberate copy it is right.
