...
class CNotchXPCHelper: NSObject, CNotchXPCHelperProtocol {
    ...
    @objc func setScreenBrightness(_ value: Float, with reply: @escaping (Bool) -> Void) {
        ...
        NSLog("CNotchXPCHelper: selected screen brightness display ID %u", displayID)
        ...
        if let io = ioServiceFor(displayID: displayID) {
            ...
            IOObjectRelease(io)
            if !ok {
                NSLog("CNotchXPCHelper: failed to set screen brightness on display ID %u", displayID)
            }
            reply(ok)
            return
        }
